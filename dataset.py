...
class Dataset:
    ...
    def save_dataset(self, project_name):
        ...
        self.normalised_df.to_csv(f"{path}/{project_name}_normalised_df.csv", index=False)

        with open(f"{path}/{project_name}_misc.csv", "w",) as file:
            file.write(f"{self.dep_feature};,")
