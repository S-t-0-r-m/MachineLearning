...
def print_single_normalised_plot(data, regs):
        ...
        for reg in regs:
            ...
            figur, ax = plt.subplots(1,2, figsize=(11, 5))
            ...
            list = clac_loss(reg.hypothesis, reg.create_feature_matrixes(df_type, size), y.to_numpy())
...
def calc_line( reges, upper_limit, lower_limit):
        ...
        for exponent in range(1,reges.get_hypothesis().size):
             x_matrx = np.vstack([ x_matrx, np.linspace(lower_limit, upper_limit, num=50) **exponent])
